...
test('water story renders its personal framing, evidence boundary and primary charts', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByText('Samoa cannot plan for an average water year.')).toBeVisible();
  await expect(page.getByText('It does not prove climate change caused these particular years.')).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'National data can identify the tests. Communities determine the response.' })).toBeVisible();
