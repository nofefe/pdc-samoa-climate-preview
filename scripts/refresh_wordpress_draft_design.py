...
def main() -> None:
    ...
    try:
        ...
        rainfall = request("/media/230?context=edit")
        sst = request("/media/231?context=edit")
        taro = request("/media/232?context=edit")
        if page.get("status") != "draft":
            raise RuntimeError("Refusing to redesign a page that is not a draft")
        leaf_url = leaf.get("source_url")
        rainfall_url = rainfall.get("source_url")
        sst_url = sst.get("source_url")
        taro_url = taro.get("source_url")
        if not all(isinstance(url, str) and url.startswith("https://") for url in (leaf_url, rainfall_url, sst_url, taro_url)):
...
<section class="pdc-section"><p class="pdc-label">01–03 · OBSERVED CONTEXT</p><h2>Average is not the year people plan for.</h2><p class="pdc-copy">Three separate annual views: Samoa-wide rainfall anomalies, Samoa EEZ SST anomalies, and Samoa taro yield. Their proximity is for context, <strong>not</strong> proof of causation.</p><figure class="pdc-chart"><img src="{rainfall_url}" alt="Annual Samoa-wide rainfall anomalies chart" /><figcaption><strong>Rainfall variability.</strong> Observed annual context only — not a forecast or local drought declaration.</figcaption></figure><figure class="pdc-chart"><img src="{sst_url}" alt="Annual Samoa EEZ sea-surface-temperature anomalies chart" /><figcaption><strong>Ocean context.</strong> EEZ aggregate, not a coastal or inshore temperature record; not proof of a relationship with rainfall.</figcaption></figure><figure class="pdc-chart"><img src="{taro_url}" alt="Annual Samoa taro yield chart" /><figcaption><strong>Taro yield.</strong> Descriptive country-level yield series, not evidence that climate caused the pattern or a forecast of harvests.</figcaption></figure></section>
...
<section class="pdc-section pdc-sources"><p class="pdc-label">SOURCES &amp; STATUS</p><p><strong>Draft only — not for public release.</strong> Source and limitation record: <a href="https://stats-sdmx-disseminate.pacificdata.org/rest/data/SPC,DF_CLIMATE_CHANGE,1.0/A.RAIN_ANOM.WS?dimensionAtObservation=AllDimensions">SPC rainfall SDMX</a> · <a href="https://stats-sdmx-disseminate.pacificdata.org/rest/data/SPC,DF_CLIMATE_CHANGE,1.0/A.SST_ANOM.WS?dimensionAtObservation=AllDimensions">SPC SST SDMX</a> · <a href="https://stats-sdmx-disseminate.pacificdata.org/rest/data/SPC,DF_AGRICULTURAL_PRODUCTION,1.0/A.WS.TARO.CROP_YIELD?dimensionAtObservation=AllDimensions">SPC taro-yield SDMX</a> · <a href="https://pafpnet.spc.int/policy-bank/countries/samoa">SPC Samoa Agriculture Policy Bank</a> · <a href="https://www.fao.org/4/y8345e/y8345e03.htm">FAO taro-leaf-blight history</a>.</p></section>
    ...
    checks = {"authenticated_user_id": me.get("id"), "page_id": after.get("id"), "page_status": after.get("status"), "editorial_shell_present": "pdc-story" in saved, "leaf_present": leaf_url in saved, "rainfall_panel_present": rainfall_url in saved, "sst_panel_present": sst_url in saved, "taro_panel_present": taro_url in saved, "taro_section_present": "START WITH TARO" in saved}
    required = ("editorial_shell_present", "leaf_present", "rainfall_panel_present", "sst_panel_present", "taro_panel_present", "taro_section_present")
    if checks["page_status"] != "draft" or not all(checks[key] for key in required):
